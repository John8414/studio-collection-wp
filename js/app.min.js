...
$(document).ready(function () {
  ...
  $(".img-scale").each(function () {
    ...
    $(this).on("mousemove", function (e) {
      ...
      img.css("transform-origin", "".concat(xPercent, "% ").concat(yPercent, "%"));
    });
  ...
  $(".image-picker").each(function () {
    var $picker = $(this);
    var $mainImage = $picker.find(".main-image");
    var $thumbnails = $picker.find(".thumbnails img");
  ...
  var lastScrollTop = 0;
  var element = $(".scroll-header");
  $(window).on("scroll", function () {
    var scrollTop = $(this).scrollTop(); // Lấy vị trí cuộn
  ...
  handleChangeColor = function handleChangeColor(price, salePrice, color, url) {
    $(".thumbnail").removeClass("item-border");
    $(".color-tags").removeClass("item-border");
    $("[data-color=".concat(color, "]")).addClass("item-border");
    $(".main-image").attr("src", url);
    $(".sale-price").text(salePrice);
    $(".original-price").text(price);
  };
  handleChangeImage = function handleChangeImage(price, salePrice, url, key) {
    $(".color-tags").removeClass("item-border");
    $(".thumbnail").removeClass("item-border");
    $(".sale-price").text(salePrice);
    $(".original-price").text(price);
    $("[data-image=".concat(key, "]")).addClass("item-border");
    $(".main-image").attr("src", url);
  };
  ...
  $(document).click(function (event) {
    if (!$(event.target).closest(".main-menu, #mobileMenuToggle, .logo-info").length) {
      $("#mobileNav").removeClass("show-menu");
      $("html").removeClass("overflow-hidden");
    }
  });
  ...
  document.addEventListener("click", function (event) {
    ...
    var isClickInsideToggle = mobileMenuToggle.contains(event.target);
  ...
  handleHideLens = function handleHideLens() {
    $(".img-zoom-lens").remove();
    $("#myresult").css("backgroundImage", "none").removeClass("d-block").addClass("d-none");
  };
  ...
  handleUpdateSearchParams = function handleUpdateSearchParams() {
    var paramsToToggle = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    ...
    setTimeout(function () {
      ...
      $.each(paramsToToggle, function (key, value) {
        ...
        if (isToggle && currentValue) {
          ...
          var _valuesArray = currentValue.split(",");
          if (_valuesArray.includes(value.toString())) {
            _valuesArray = _valuesArray.filter(function (val) {
              return val !== value.toString();
            });
          } else {
            _valuesArray.push(value.toString());
          }
          if (_valuesArray.length > 0) {
            searchParams.set(key, _valuesArray.join(","));
          } else {
            searchParams["delete"](key);
          }
        } else {
